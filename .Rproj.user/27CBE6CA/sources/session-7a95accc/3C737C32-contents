...
server <- function(input, output, session) {
  ...
  if (file.exists(tower)) {
    ...
    tower_data <- data.frame(Age = numeric(), Blocks = numeric(), Color = character())
  }
  ...
  output$scatterPlot <- renderPlotly({
    ...
    p <- ggplot(plot_data, aes(y = Blocks, x = factor(Age), color = Color, text = paste("<br>Age:", Age, "<br>Blocks:", Blocks))) +
